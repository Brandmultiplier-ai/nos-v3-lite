...
function makeApex(range: "7d" | "30d" | "90d"): ClientData {
  ...
  return {
    ...
    recommendedActions: range === "7d" ? [
      {
        priority: "high",
        icon: "Building2",
        title: "Capitalize on this week's exec LinkedIn momentum",
    ...
    ] : [
      {
        ...
        description: "This quarter's data confirms: executive posts drive 78% of deal-influencing interactions. Formalizing a 12-week rolling executive narrative program would systematize this into a reliable pipeline engine.",
        cta: "Design program",
      },
      {
        priority: "medium",
    ...
    positioning: {
      tldr: {
        summary: range === "7d"
          ? "This week, EnterpriseLayer published a CIO-targeting LinkedIn series with strong early engagement. Monitor whether this shifts their narrative share-of-voice score before your executive posts go out Thursday — timing your response will maximize counter-narrative visibility."
          : range === "30d"
          ? "Apex Systems holds the lower edge of the Leaders quadrant with strong enterprise credibility, but EnterpriseLayer sits above it on both axes. The gap is primarily in brand Impact — buyers recognize Apex but don't yet instinctively associate it as the definitive enterprise IT narrative platform. Closing this gap will directly improve win rates in competitive deals."
        ...
        actions: range === "7d" ? [
          { title: "Time your executive response to EnterpriseLayer", description: "EnterpriseLayer posted a CIO series this week. Scheduling your exec post for Thursday morning — peak CIO LinkedIn engagement window — will maximize counter-narrative visibility.", priority: "high", cta: "Schedule post" },
          ...
          { title: "Increase executive visibility", description: "Enterprise buyers want to buy from recognized experts. Increase exec LinkedIn presence to 4 posts per week.", priority: "high", cta: "Build exec calendar" },
      ...
      movementTimeline: (() => {
        ...
        const steps = [{ x: 48, y: 52 }, { x: 52, y: 56 }, { x: 55, y: 60 }, { x: 57, y: 63 }, { x: 60, y: 66 }, { x: 62, y: 68 }];
    ...
    search: {
      tldr: {
        ...
        actions: range === "7d" ? [
          ...
          { title: "Optimize the top featured snippet opportunity", description: "Your highest-traffic page is position 4 for a featured snippet that sends 340 monthly visits to a competitor. A concise definition section addition could capture it this week.", priority: "high", cta: "Update page" },
          ...
          { title: "Optimize for featured snippets", description: "12 ranking pages are in position 4–7 with featured snippet opportunities. Add concise definition sections to capture them.", priority: "medium", cta: "Optimize pages" },
        ] : [
      ...
      healthScore: 92,
      domainRating: 68,
      organicKeywordsTotal: { value: 6200, change: 9, sparkline: spark() },
      backlinks: { value: 41200, change: 35, sparkline: spark() },
      referringDomains: { value: 3140, change: 128, sparkline: spark() },
      ...
      keywordBuckets: [
        ...
        { bucket: "11-50", label: "#11–50", count: 643, change: 112 },
        { bucket: "51-100", label: "#51–100", count: 174, change: 47 },
      ],
      countryBreakdown: [
        { code: "US", name: "United States", flag: "🇺🇸", traffic: Math.round(9000 * mult), trafficChange: 1400, lat: 38, lon: -97 },
        { code: "IN", name: "India",          flag: "🇮🇳", traffic: Math.round(2700 * mult), trafficChange: 310,  lat: 20, lon: 77 },
        { code: "DE", name: "Germany",        flag: "🇩🇪", traffic: Math.round(1800 * mult), trafficChange: 240,  lat: 51, lon: 10 },
        { code: "GB", name: "United Kingdom", flag: "🇬🇧", traffic: Math.round(1600 * mult), trafficChange: 180,  lat: 51, lon: -1 },
        { code: "CA", name: "Canada",         flag: "🇨🇦", traffic: Math.round(1300 * mult), trafficChange: 150,  lat: 56, lon: -106 },
        { code: "JP", name: "Japan",          flag: "🇯🇵", traffic: Math.round(1000 * mult), trafficChange: 90,   lat: 36, lon: 138 },
        { code: "NL", name: "Netherlands",    flag: "🇳🇱", traffic: Math.round(900 * mult),  trafficChange: 70,   lat: 52, lon: 5 },
      ],
      keywords: [
      ...
      pipelineFromOrganic: { value: Math.round(96000 * mult), change: range === "7d" ? 14 : range === "30d" ? 38 : 62, sparkline: spark(), prefix: "$" },
      geoEngines: [
    ...
    website: {
      ...
      readingBehavior: [
        { label: "Casual", pct: 67.4, sessions: Math.round(11600 * mult), color: "#7C7FFF" },
        { label: "Serious", pct: 22.8, sessions: Math.round(3900 * mult), color: "#FBBF24" },
        { label: "Reader", pct: 9.8, sessions: Math.round(1700 * mult), color: "#34D399" },
      ],
      ...
      funnelSteps: [
        { label: "Homepage", sessions: Math.round(17200 * mult), dropoffPct: 0 },
        { label: "Enterprise / Product", sessions: Math.round(9976 * mult), dropoffPct: 42 },
        { label: "Pricing", sessions: Math.round(4290 * mult), dropoffPct: 57 },
        { label: "Demo Request", sessions: Math.round(1459 * mult), dropoffPct: 66 },
        { label: "Confirmed", sessions: Math.round(554 * mult), dropoffPct: 62 },
      ],
      sessionRecordings: [
    ...
    content: {
      tldr: {
        summary: range === "7d"
          ...
          : "Content has been Apex's highest-ROI channel this quarter, generating $480k in LinkedIn-attributed pipeline and $280k from the newsletter. Executive thought leadership has become a recognized signal in the enterprise IT community — buyer-side references to Apex content in deal conversations increased 34% this quarter.",
        actions: range === "7d" ? [
          ...
          { title: "Follow up with newsletter openers from Tuesday", description: "38% of Tuesday's newsletter readers opened — 12 are active opportunities. A personalized one-line follow-up from the AE today would be contextually relevant.", priority: "high", cta: "Send follow-ups" },
          ...
          { title: "Formalize the executive content program", description: "This quarter's data confirms executive posts are Apex's highest-ROI content investment. Formalize a 12-week rolling executive narrative program to sustain and scale the output.", priority: "high", cta: "Design program" },
      ...
      socialOverview: [
        ...
        { channel: "Email", posts: Math.round(3 * mult), reach: Math.round(6000 * mult), engagementRate: 36, pipeline: Math.round(280000 * mult) },
      ],
      topPosts: [
      ...
      platformStats: [
        { platform: "LinkedIn", color: "#0A66C2", followers: 18000, followersChange: 14, reach: Math.round(62000 * mult), engagementRate: 5.4, posts: Math.round(8 * mult) },
        { platform: "X / Twitter", color: "#FFFFFF", followers: 8400, followersChange: 8, reach: Math.round(24000 * mult), engagementRate: 2.8, posts: Math.round(18 * mult) },
        { platform: "YouTube", color: "#FF0000", followers: 4200, followersChange: 22, reach: Math.round(18000 * mult), engagementRate: 6.8, posts: Math.round(4 * mult), reelsWatchTime: 42 },
        { platform: "Newsletter", color: "#34D399", followers: 6000, followersChange: 11, reach: Math.round(6000 * mult), engagementRate: 36, posts: Math.round(3 * mult) },
      ],
      audienceGrowthStacked: Array.from({ length: 12 }, (_, i) => {
        ...
        return {
          ...
          linkedin: Math.round(14200 + i * 350 + Math.sin(i * 0.4) * 120),
          x: Math.round(6200 + i * 190 + Math.cos(i * 0.4) * 60),
          youtube: Math.round(2600 + i * 145 + Math.sin(i * 0.5) * 40),
          newsletter: Math.round(3800 + i * 200 + Math.cos(i * 0.3) * 30),
        };
      }),
      sentiment: {
      ...
      linkedinKPIs: {
        followers:    { value: 18000, change: 14.2, sparkline: spark() },
    ...
    outreach: {
      tldr: {
        ...
        actions: range === "7d" ? [
          ...
          { title: "Review response handling for this week's pending replies", description: "3 replies from this week are awaiting a response. Use the enterprise-specific response script to maximize meeting booking rate from each.", priority: "medium", cta: "Send responses" },
        ] : range === "30d" ? [
          ...
          { title: "Standardize LinkedIn-first approach", description: "Reps who send a LinkedIn view before email day 1 see 2.1× reply rates. Make this standard practice across all AEs.", priority: "high", cta: "Update playbook" },
          ...
          { title: "Formalize the LinkedIn-first playbook", description: "The LinkedIn-first warm-up strategy is proven across a full quarter. Document it as a mandatory AE standard: LinkedIn view on day 0, email on day 1. Train all reps and measure compliance.", priority: "high", cta: "Formalize playbook" },
      ...
      masterTrend: dates.map((date, i) => {
        const base = Math.round(1200 + Math.sin(i * 0.5) * 400 + i * 15);
        return { date, sent: base, opens: Math.round(base * 0.63), uniqueOpens: Math.round(base * 0.56), replies: Math.round(base * 0.068) };
      }),
      emailCampaigns: [
        {
          id: "e1", name: "CISO — Cybersecurity Narrative Sequence", status: "active", sequence: 5,
          sent: Math.round(8400 * mult), opens: Math.round(3200 * mult), replies: Math.round(680 * mult), meetings: Math.round(150 * mult), pipeline: Math.round(580000 * mult),
          openRate: 38.1, replyRate: 8.1, positiveReplyRate: 5.2, bounceRate: 1.4,
          leads: Math.round(8400 * mult), completed: Math.round(52 * mult), bounced: Math.round(8 * mult), unsubscribed: Math.round(4 * mult),
          sequenceSteps: [
        ...
        },
        {
          ...
          sent: Math.round(6200 * mult), opens: Math.round(2071 * mult), replies: Math.round(428 * mult), meetings: Math.round(94 * mult), pipeline: Math.round(380000 * mult),
          openRate: 33.4, replyRate: 6.9, positiveReplyRate: 4.1, bounceRate: 1.6,
          leads: Math.round(6200 * mult), completed: Math.round(38 * mult), bounced: Math.round(6 * mult), unsubscribed: Math.round(3 * mult),
          sequenceSteps: [
        ...
        },
        {
          ...
          sent: Math.round(4000 * mult), opens: Math.round(1096 * mult), replies: Math.round(152 * mult), meetings: Math.round(34 * mult), pipeline: Math.round(180000 * mult),
          openRate: 27.4, replyRate: 3.8, positiveReplyRate: 2.2, bounceRate: 2.0,
          leads: Math.round(4000 * mult), completed: Math.round(68 * mult), bounced: Math.round(6 * mult), unsubscribed: Math.round(3 * mult),
          sequenceSteps: [
      ...
      emailFunnel: [
        ...
        { stage: "Opps",      count: Math.round(68 * mult),    rate: 24.5, color: "#34D399" },
      ],
      crmPipelineFunnel: [
        { stage: "Request For Info", value: Math.round(2400000 * mult), deals: Math.round(58 * mult), pct: 100,  color: "#0EA5E9" },
        { stage: "Presentation",     value: Math.round(1820000 * mult), deals: Math.round(44 * mult), pct: 75.8, color: "#6366F1" },
        { stage: "Qualified",        value: Math.round(1380000 * mult), deals: Math.round(33 * mult), pct: 57.5, color: "#A78BFA" },
        { stage: "Negotiation",      value: Math.round(980000 * mult),  deals: Math.round(24 * mult), pct: 40.8, color: "#FBBF24" },
        { stage: "Won",              value: Math.round(1302000 * mult), deals: Math.round(14 * mult), pct: 24.2, color: "#34D399" },
        { stage: "Lost",             value: Math.round(240000 * mult),  deals: Math.round(6 * mult),  pct: 10.0, color: "#FF4455" },
      ],
      replyWaterfall: [
    ...
    integrations: [
      ...
      { id: "demandbase", name: "Demandbase", category: "paid-media", connected: true, lastSync: "15 min ago" },
      ...
      { id: "techtarget", name: "TechTarget", category: "paid-media", connected: true, lastSync: "40 min ago" },
      { id: "gartner-digital-markets", name: "Gartner Digital Markets", category: "paid-media", connected: true, lastSync: "1 hour ago" },
      { id: "reddit-ads", name: "Reddit Ads", category: "paid-media", connected: false },
    ],
    paidMedia: {
      tldr: {
        summary: range === "7d"
          ? "Apex's paid media engine is running at enterprise scale across 4 connected platforms. LinkedIn Ads delivered 4.7× ROAS this week targeting VP Engineering buyers. Programmatic ABM against the named-account list is fueling 38% of all demo bookings from paid."
          : range === "30d"
          ? "Enterprise paid media delivered a 3.9× blended ROAS this month, above benchmark for enterprise B2B infrastructure. LinkedIn continues to be the highest-quality channel for enterprise buyers, contributing 41% of enterprise pipeline from paid sources. Programmatic ABM against the named-account list is the fastest-improving line, with cost-per-engaged-account down 18% quarter over quarter."
          : "Paid media delivered a 3.9× blended ROAS this quarter across 4 platforms. LinkedIn Ads is the top performer among all channels — directly attributable to precise Tier-1 enterprise account targeting. Programmatic ABM against the named-account list continued to improve efficiency, with cost-per-engaged-account down 18% quarter over quarter. Recommend increasing LinkedIn budget by 35% in Q3.",
        actions: range === "7d" ? [
          ...
          { title: "Expand programmatic ABM to 22 new accounts", description: "22 Tier-1 accounts showed website intent signals this week but are not yet in the programmatic ABM audience. Sync the account list to create a high-value targeting segment.", priority: "high", cta: "Activate" },
          { title: "Test an analyst-adjacent placement this week", description: "A single-week test in an analyst newsletter would establish an early cost-per-qualified-account benchmark ahead of next month's budget review.", priority: "medium", cta: "Plan test" },
        ] : range === "30d" ? [
          ...
          { title: "Implement cross-platform attribution modeling", description: "With 4 platforms active, cross-channel attribution is increasingly important. A data-driven attribution model would improve budget allocation by up to 22% efficiency gain.", priority: "high", cta: "Build model" },
          { title: "Expand analyst-adjacent placements", description: "Gartner and Forrester co-marketing placements reach the CIO shortlist at the evaluation stage. A $12k per month test across analyst newsletters and webinar sponsorships has an estimated $180 cost per qualified account in your category.", priority: "medium", cta: "Plan test" },
        ] : [
          { title: "Build an enterprise paid media playbook", description: "Apex's paid media performance this quarter establishes clear channel benchmarks. Documenting a replicable playbook — audience targeting, creative strategy, budget allocation — would systematize the approach for sustained performance.", priority: "high", cta: "Build playbook" },
          ...
          { title: "Scale the analyst-adjacent placement test", description: "Early analyst newsletter and webinar sponsorship placements are tracking well below the enterprise CAC benchmark. A quarterly commitment across Gartner and Forrester channels would compound the citation and pipeline effect.", priority: "medium", cta: "Plan expansion" },
        ],
      },
      ...
      campaigns: [
        ...
        { id: "c2", name: "VP Sales Sequence — LinkedIn", platform: "LinkedIn Ads", status: "active", spend: Math.round(8000 * mult), revenue: Math.round(33000 * mult), roas: 4.1, impressions: Math.round(92000 * mult), clicks: Math.round(3680 * mult), conversions: Math.round(58 * mult), cpa: 138, cpc: 2.17 },
        { id: "c3", name: "CXO Account Targeting — LinkedIn", platform: "LinkedIn Ads", status: "active", spend: Math.round(30700 * mult), revenue: Math.round(126810 * mult), roas: 4.1, impressions: Math.round(310000 * mult), clicks: Math.round(12280 * mult), conversions: Math.round(198 * mult), cpa: 155, cpc: 2.50 },
        { id: "c4", name: "Named-Account Programmatic ABM", platform: "Programmatic ABM", status: "active", spend: Math.round(13500 * mult), revenue: Math.round(45900 * mult), roas: 3.4, impressions: Math.round(2400000 * mult), clicks: Math.round(28800 * mult), conversions: Math.round(52 * mult), cpa: 260, cpc: 0.47 },
        { id: "c5", name: "Enterprise Pipeline — Google Search", platform: "Google Search", status: "active", spend: Math.round(8000 * mult), revenue: Math.round(23200 * mult), roas: 2.9, impressions: Math.round(160000 * mult), clicks: Math.round(6400 * mult), conversions: Math.round(48 * mult), cpa: 167, cpc: 1.25 },
        { id: "c6", name: "Gartner Newsletter Placement", platform: "Analyst and Trade Media", status: "active", spend: Math.round(2500 * mult), revenue: Math.round(5250 * mult), roas: 2.1, impressions: Math.round(420000 * mult), clicks: Math.round(2100 * mult), conversions: Math.round(10 * mult), cpa: 250, cpc: 1.19 },
        { id: "c7", name: "Forrester Webinar Sponsorship", platform: "Analyst and Trade Media", status: "active", spend: Math.round(1500 * mult), revenue: Math.round(3150 * mult), roas: 2.1, impressions: Math.round(180000 * mult), clicks: Math.round(900 * mult), conversions: Math.round(6 * mult), cpa: 250, cpc: 1.67 },
      ],
      platformBreakdown: [
        { platform: "LinkedIn Ads", spend: Math.round(56700 * mult), revenue: Math.round(243810 * mult), roas: 4.3, color: "#0A66C2" },
        { platform: "Programmatic ABM", spend: Math.round(13500 * mult), revenue: Math.round(45900 * mult), roas: 3.4, color: "#8B5CF6" },
        { platform: "Google Search", spend: Math.round(8000 * mult), revenue: Math.round(23200 * mult), roas: 2.9, color: "#4285F4" },
        { platform: "Analyst and Trade Media", spend: Math.round(4000 * mult), revenue: Math.round(8400 * mult), roas: 2.1, color: "#64748B" },
      ],
      spendTrend: dates.map((date, i) => ({
        date,
        spend: Math.round((4342 + Math.sin(i * 0.5) * 1002 + i * 60) * mult),
        revenue: Math.round((17127 + Math.cos(i * 0.4) * 3071 + i * 240) * mult),
      })),
      bestCampaigns: [
        { name: "Tier-1 ABM — LinkedIn", platform: "LinkedIn Ads", revenue: Math.round(84000 * mult), conversions: Math.round(124 * mult), roas: 4.7 },
        { name: "CXO Account Targeting — LinkedIn", platform: "LinkedIn Ads", revenue: Math.round(126810 * mult), conversions: Math.round(198 * mult), roas: 4.1 },
        { name: "Named-Account Programmatic ABM", platform: "Programmatic ABM", revenue: Math.round(45900 * mult), conversions: Math.round(52 * mult), roas: 3.4 },
      ],
      bestAds: [
        ...
        { name: "CXO Account List — Static", platform: "LinkedIn Ads", revenue: Math.round(39800 * mult), conversions: Math.round(68 * mult), ctr: 4.9 },
        { name: "Named-Account Retargeting — Display", platform: "Programmatic ABM", revenue: Math.round(29700 * mult), conversions: Math.round(52 * mult), ctr: 1.2 },
      ],
    },
    narrativeIntel: {
      ...
      phaseMetrics: {
        phase1: {
          growthMetric: { name: "Market perception", value: "+6%", change: 6, suppressChange: true },
          emotionalIndicator: { name: "Sentiment score", value: "48/100", change: 3 },
        },
        phase2: {
          growthMetric: { name: "Lead conversion", value: "+9%", change: 9, suppressChange: true },
        ...
        phase4: {
          growthMetric: { name: "Deal size", value: "+7%", change: 7, suppressChange: true },
