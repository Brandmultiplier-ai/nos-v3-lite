...
const BRAND_ICONS: Record<string, { path: string; glyph: string; tile?: string; gradient?: boolean }> = {
...
function BrandLogo({ id, size = 32 }: { id: string; size?: number }) {
    ...
    return (
      <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
        {icon.gradient ? (
          ...
          <rect width="32" height="32" rx="7" fill={icon.tile} />
        )}
  ...
  switch (id) {
    ...
    /* Google Analytics — official multicolor (kept) */
    case "analytics":
      return (
        <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
          <rect width="32" height="32" rx="7" fill="white" stroke="#E5E7EB" />
